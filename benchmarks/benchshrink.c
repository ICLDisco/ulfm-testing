...
int main(int argc, char *argv[])
{
    ...
    comms = malloc(sizeof(MPI_Comm)*(before+after+2));
    ...
    for(i = 0; i < before; i++) {
        start = MPI_Wtime();
        ret = MPIX_Comm_shrink(MPI_COMM_WORLD, &comms[i]);
    ...
    start = MPI_Wtime();
    ret = MPIX_Comm_shrink(MPI_COMM_WORLD, &comms[before]);
    ...
    start = MPI_Wtime();
    ret = MPIX_Comm_shrink(MPI_COMM_WORLD, &comms[before+1]);
    ...
    for(i = 0; i < after; i++) {
        start = MPI_Wtime();
        ret = MPIX_Comm_shrink(MPI_COMM_WORLD, &comms[i+before+2]);
