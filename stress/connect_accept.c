...
int main(int argc, char *argv[]) {
    ...
    MPI_Info_get(MPI_INFO_ENV, "args", MPI_MAX_INFO_VAL, args, &flag);
    ...
    char* arg_array[2] = { "it=1", NULL };
    int err_array[3] = { MPI_SUCCESS };

    flag = MPI_Comm_spawn(command, arg_array, 3, MPI_INFO_NULL, 0, MPI_COMM_WORLD, &icomm, err_array);
    if( MPI_SUCCESS != flag ) {
