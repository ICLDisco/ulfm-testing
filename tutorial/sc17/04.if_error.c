...
int main( int argc, char* argv[] ) {
    ...
    for( i = 0; i < 10; i++ ) {
        ...
        if( rc != MPI_SUCCESS ) {
            /* Ok, some error occured, force other processes to exit the loop
             * because when I am leaving, I will not match the sendrecv, and
             * that would cause them to deadlock */
