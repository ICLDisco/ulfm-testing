...
int main( int argc, char* argv[] ) {
    ...
    if( MPI_COMM_NULL == world ) {
        do {
            /* unused spares are always ready to complete */
            int completed = 1;
            MPIX_Comm_agree( MPI_COMM_WORLD, &completed );
            if( completed ) goto finalize;
    ...
    /* Unlock the spares, completed=0 so they will join us */
    MPIX_Comm_agree( MPI_COMM_WORLD, &completed );
...
joinwork:
    ...
    /* Unlock the spares so they can join Finalize */
    completed = 1;
    MPIX_Comm_agree(MPI_COMM_WORLD, &completed);
finalize:
    MPI_Finalize();
